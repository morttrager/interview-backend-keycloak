import { Injectable } from '@angular/core';
import { HttpResponse } from '@angular/common/http';
import { Resolve, ActivatedRouteSnapshot, Routes, Router } from '@angular/router';
import { Observable, of, EMPTY } from 'rxjs';
import { flatMap } from 'rxjs/operators';

import { Authority } from 'app/shared/constants/authority.constants';
import { UserRouteAccessService } from 'app/core/auth/user-route-access-service';
import { IInterviewee, Interviewee } from 'app/shared/model/interviewee.model';
import { IntervieweeService } from './interviewee.service';
import { IntervieweeComponent } from './interviewee.component';
import { IntervieweeDetailComponent } from './interviewee-detail.component';
import { IntervieweeUpdateComponent } from './interviewee-update.component';

@Injectable({ providedIn: 'root' })
export class IntervieweeResolve implements Resolve<IInterviewee> {
  constructor(private service: IntervieweeService, private router: Router) {}

  resolve(route: ActivatedRouteSnapshot): Observable<IInterviewee> | Observable<never> {
    const id = route.params['id'];
    if (id) {
      return this.service.find(id).pipe(
        flatMap((interviewee: HttpResponse<Interviewee>) => {
          if (interviewee.body) {
            return of(interviewee.body);
          } else {
            this.router.navigate(['404']);
            return EMPTY;
          }
        })
      );
    }
    return of(new Interviewee());
  }
}

export const intervieweeRoute: Routes = [
  {
    path: '',
    component: IntervieweeComponent,
    data: {
      authorities: [Authority.USER],
      pageTitle: 'Interviewees',
    },
    canActivate: [UserRouteAccessService],
  },
  {
    path: ':id/view',
    component: IntervieweeDetailComponent,
    resolve: {
      interviewee: IntervieweeResolve,
    },
    data: {
      authorities: [Authority.USER],
      pageTitle: 'Interviewees',
    },
    canActivate: [UserRouteAccessService],
  },
  {
    path: 'new',
    component: IntervieweeUpdateComponent,
    resolve: {
      interviewee: IntervieweeResolve,
    },
    data: {
      authorities: [Authority.USER],
      pageTitle: 'Interviewees',
    },
    canActivate: [UserRouteAccessService],
  },
  {
    path: ':id/edit',
    component: IntervieweeUpdateComponent,
    resolve: {
      interviewee: IntervieweeResolve,
    },
    data: {
      authorities: [Authority.USER],
      pageTitle: 'Interviewees',
    },
    canActivate: [UserRouteAccessService],
  },
];
